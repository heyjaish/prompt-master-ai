...
export default function LoginPage() {
  ...
  const handleSkillsark = () => {
    ...
    // Build the callback URL dynamically from current origin
    const callbackUrl = `${window.location.origin}/auth/callback`;
    const redirectUrl = `${SKILLSARK_PORTAL}/auth-gate.php?return_to=${encodeURIComponent(callbackUrl)}`;
    window.location.href = redirectUrl;
    // Note: setSLoading stays true — page will navigate away
  };
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <button
              ...
              style={{
                ...
                border: "1px solid var(--border-lg)",
                color: "var(--tx-1)", fontSize: 14, fontWeight: 600,
                cursor: isLoading ? "not-allowed" : "pointer",
                opacity: sLoading ? .45 : 1,
                transition: "all .18s",
              }}
            ...
            {/* ── Skillsark SSO button ──────────── */}
            <button
              onClick={handleSkillsark}
              disabled={isLoading}
              style={{
                width: "100%",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
                ...
                cursor: isLoading ? "not-allowed" : "pointer",
                opacity: gLoading ? .45 : 1,
                transition: "all .18s",
              }}
              onMouseEnter={e => { if (!isLoading) { e.currentTarget.style.background = "linear-gradient(135deg, rgba(99,102,241,.28), rgba(139,92,246,.2))"; e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(99,102,241,.2)"; } }}
              onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(99,102,241,.18), rgba(139,92,246,.12))"; e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "none"; }}
            >
              {sLoading ? <Loader2 size={17} className="spin" /> : (
                /* Skillsark "S" logo mark */
                <div style={{
