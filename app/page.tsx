...
const VIEW_PREF_KEY = "pm_view_pref"; // localStorage key for view preference
...
export default function HomePage() {
  ...
  // ✅ FIX: View mode is user-controlled. Never auto-switch after generation.
  const [viewMode, setViewMode]           = useState<ViewMode>("chat");
  const [userSetView, setUserSetView]     = useState(false); // true once user manually changed
  ...
  const [inputCollapsed, setInputCollapsed] = useState(false); // input auto-collapse after gen
  ...
  // ── View change — always user-driven ───────────────────────
  ...
  const handleSend = useCallback(async (idea: string, images: UploadedImage[]) => {
    ...
    setIsLoading(true);
    setInputCollapsed(true); // collapse input while generating
    ...
    // Contextual memory: last 3 prompts in same specialist
    ...
    try {
      ...
      // ✅ FIX: Update message, stop spinner — do NOT auto-switch view
      ...
      // ✅ REMOVED: setViewMode("split") — user controls this

      // Save to history (specialist-specific)
      ...
      setInputCollapsed(false); // re-expand on error
    } finally {
      setIsLoading(false); // ✅ FIX: always stop spinner
    }
  ...
  const handleSelectHistory = (entry: HistoryEntry) => {
    ...
    // ✅ FIX: Respect user's view preference when loading history
    if (!userSetView) {
      setViewMode("split");
      localStorage.setItem(VIEW_PREF_KEY, "split");
    }
  ...
  const handleNewChat = () => {
    setMessages([]); setSelectedId(undefined); setLatestPrompt(null);
    setInputCollapsed(false);
    // ✅ Don't reset viewMode — persist user's preferred state
  };

  const handleChipClick = (word: string) => {
    setAppendText(word);
    setInputCollapsed(false); // expand input so user can see what was added
    if (user) trackEvent({ uid: user.uid, event: "chip_clicked", metadata: { word } });
  };

  const handleSpecialistActivate = (s: Specialist | null) => {
    setSpecialist(s);
    setChipsRefresh(n => n + 1); // reload chips for this specialist
  ...
  // Sidebar history: specialist-specific or global
  const sidebarHistory = activeSpecialist
    ? history.filter(h => h.specialist === activeSpecialist.slotId)
    : history.filter(h => !h.specialist); // global = no specialist attached
  ...
  return (
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: "var(--bg)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
        ...
        {/* ✅ View toggle — only shown when there's content, user-controlled ONLY */}
        {latestPrompt && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 16px", borderBottom: "1px solid var(--border)", background: "rgba(17,17,22,0.7)", backdropFilter: "blur(12px)", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          {viewMode === "split" && latestPrompt ? (
            ...
            <div className="flex-1 scroll">
              {isEmpty ? (
                ...
                <div style={{ maxWidth: 760, margin: "0 auto", width: "100%", padding: "32px 20px 20px", display: "flex", flexDirection: "column", gap: 28 }}>
          ...
          <div style={{
            ...
            padding: inputCollapsed ? "6px 20px" : "10px 20px 14px",
            ...
            <div style={{ maxWidth: 760, margin: "0 auto", width: "100%", display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {inputCollapsed ? (
                <button
                  ...
                  style={{
                    width: "100%", padding: "8px 16px", borderRadius: 12,
                    background: "rgba(255,255,255,.04)", border: "1px dashed var(--border)",
                    color: "var(--tx-3)", fontSize: 13, cursor: "pointer",
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                    transition: "all .15s",
                  }}
                  onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,.07)"; e.currentTarget.style.borderColor = "rgba(99,102,241,.4)"; }}
                  onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,.04)"; e.currentTarget.style.borderColor = "var(--border)"; }}
                >
                  <Zap size={13} color="var(--accent-fg)"/>
                  <span>Type another prompt…</span>
                </button>
              ) : (
                <>
                  {/* Learning chips — specialist-scoped */}
                  {user && (
                    <LearningChips
                      uid={user.uid}
